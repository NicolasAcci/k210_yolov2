...
def create_feature_extractor(architecture, input_size, alpha):
    """
    # Args
        architecture : str
        input_size : int
    ...
    if architecture == 'Inception3':
        ...
        feature_extractor = MobileNetFeature(input_size, weights, alpha)
...
class MobileNetFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size, weights=False, alphaa=0.75):
        input_image = Input(shape=(input_size, input_size, 3))
        if alphaa == "1.0":
            alphaaa = 1.0
        elif alphaa == "0.75":
            ...
            alphaaa = 0.25
        mobilenet = MobileNet(input_shape=(224, 224, 3), alpha=alphaaa, depth_multiplier=1, dropout=0.001,
